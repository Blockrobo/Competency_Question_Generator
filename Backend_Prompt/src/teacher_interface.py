...
def interactive_teacher_setup():
    ...
    print("-" * 70)
    cycles = get_cycles(config.subject)
    ...
    print(f"\n📖 {labels['step3']}")
    print("-" * 70)
    topics = get_topics(config.subject, config.cycle)
    for i, topic in enumerate(topics, 1):
        # Show abbreviated version
        topic_parts = topic.split(" - ")
        if len(topic_parts) > 0:
            print(
                f"  {i}. {topic_parts[0]} - {topic_parts[1] if len(topic_parts) > 1 else ''}"
            )
        else:
            print(f"  {i}. {topic}")
